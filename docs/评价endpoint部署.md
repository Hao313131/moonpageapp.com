...
| 真实浏览器响应形状矩阵 | **7 / 7 通过**（注入 7 种响应体断言 UI 终态） |
| 判定逻辑单元用例表 | **22 / 22 通过**，5 种成功形状 **0 例误判为失败** |

### 响应判定为什么是「容错」的

`responseAccepted()`（`lib/reviewEndpoint.ts`）**不是**简单判断 `res.ok`，也**不是**精确匹配 `"true"`：

1. **不能只看状态码** —— 未激活时它回 200，只看状态码会把「被丢弃」当成「收到了」。
2. **也不能精确匹配 `"true"`** —— 我们只实测到过**拒绝**形状（`"false"`），
   **接受**形状在激活前无法触发。万一真实返回是布尔 `true` 而不是字符串，
   精确匹配会让**每一次真实提交都显示报错**，那是这个表单最不能出的故障。

所以判定顺序是：

| 顺序 | 规则 |
|---|---|
| 1 | `kind: "none"`（表单被显式关掉）→ **失败** |
| 2 | `kind: "json"`（自定义端点）→ **成功**（2xx 就是契约） |
| 3 | 正文为空 → **失败** |
| 4 | 依次读 `success` → `ok` → `status`，把布尔 / 数字 / `"true"/"1"/"yes"/"ok"` 归为真，<br>`false`/`0`/`"no"/"error"/"fail"` 归为假 |
| 5 | 没有明确表态（或正文不是 JSON）→ **信 HTTP 2xx**，<br>除非正文含已实测到的拒绝用语（`needs activation` / `captcha` / `blacklist` / `rate limit` 等） |

> **为什么这样最安全**：第 5 条把「未知形状」判为成功，看似宽松，
> 但**判为成功与判为失败的代价不对称** ——
> 判失败只是多显示一个邮件兜底链接（评价照样到手，人只是多走一步）；
> 判成功而实际没送达才会丢评价。而第 5 条只会在「服务商回了 2xx 且没说任何拒绝理由」时触发，
> 这种情况下真丢件的概率极低；真正会丢件的两种已知情况（未激活、垃圾内容）都在第 4/5 条被拦住了。
> 更重要的是：**改动只可能消掉误报，不可能弄丢评价。**
